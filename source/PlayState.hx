...
class PlayState extends MusicBeatState
{
	...
	public var cameraMoveTween:FlxTween;

	public var cameraNoteTween:FlxTween;
	...
	public var ratingZoomTween:FlxTween;
	...
	public var cameraSpeed:Float = 0.4;
	...
	public var screenshader:Shaders.GlitchEffect;
	...
	public var scoreDataTxt:FlxText;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'benbi' :
				...
				var bg:BGSprite = new BGSprite('benbi/BG', -100, 0, 0.5, 0.5);
				bg.scale.set(1.1, 1.1);
				trace('we at benbi rn btw so just you know');
			...
			case 'babi':
				...
				var bg:BGSprite = new BGSprite('babi/BG', -100, 0, 0.5, 0.5);
		...
		timeTxt.setFormat(Paths.font("quicksand_bold.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("quicksand_bold.otf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		scoreDataTxt = new FlxText(10, FlxG.height - 30, FlxG.width, SONG.song +' - '+ CoolUtil.difficultyString() + ' - Composed By ' + Metadata.getAuthor(SONG.song.toLowerCase()), 20);
		scoreDataTxt.setFormat(Paths.font("quicksand_bold.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreDataTxt.scrollFactor.set();
		scoreDataTxt.borderSize = 1.25;
		scoreDataTxt.visible = !ClientPrefs.hideHud;
		add(scoreDataTxt);

		sickTxt = new FlxText(20, 0, FlxG.width, "Sicks!: 0", 20);
		sickTxt.setFormat(Paths.font("quicksand_bold.otf"), 30, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sickTxt.scrollFactor.set();
		sickTxt.borderSize = 1.25;
		sickTxt.visible = !ClientPrefs.hideHud;
		sickTxt.screenCenter(Y);
		sickTxt.y -= 50;

		goodTxt = new FlxText(20, 0, FlxG.width, "Goods: 0", 20);
		goodTxt.setFormat(Paths.font("quicksand_bold.otf"), 30, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		goodTxt.scrollFactor.set();
		goodTxt.borderSize = 1.25;
		goodTxt.visible = !ClientPrefs.hideHud;
		goodTxt.y = sickTxt.y + sickTxt.height + 5;

		badTxt = new FlxText(20, 0, FlxG.width, "Bads: 0", 20);
		badTxt.setFormat(Paths.font("quicksand_bold.otf"), 30, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		badTxt.scrollFactor.set();
		badTxt.borderSize = 1.25;
		badTxt.visible = !ClientPrefs.hideHud;
		badTxt.y = goodTxt.y + goodTxt.height + 5;

		shitTxt = new FlxText(20, 0, FlxG.width, "Shits: 0", 20);
		shitTxt.setFormat(Paths.font("quicksand_bold.otf"), 30, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		shitTxt.scrollFactor.set();
		shitTxt.borderSize = 1.25;
		shitTxt.visible = !ClientPrefs.hideHud;
		shitTxt.y = badTxt.y + badTxt.height + 5;

		// incredibly awful code ill rewrite it later dont LOOK AT THIS THERES NOTHING HERE DONT LOOK DONT LOOK

		sickTxt.centerOrigin();
		goodTxt.centerOrigin();
		badTxt.centerOrigin();
		shitTxt.centerOrigin();
		...
		botplayTxt.setFormat(Paths.font("quicksand_bold.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function reloadHealthBarColors() {
		...
		healthBar.createImageBar(Paths.image('healthbars/' + dad.healthBarFile),
			Paths.image('healthbars/' + boyfriend.healthBarFile));


		healthBar.updateBar();
	}

	public function overrideHealthBar(dad, bf) {
		healthBar.createImageBar(Paths.image('healthbars/' + dad),
			Paths.image('healthbars/' + bf));
		healthBar.updateBar();
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (healthBar.percent > 80 && iconP1.has3rdanim)
		...
		if (healthBar.percent < 20 && iconP2.has3rdanim)
	...
	public function moveCamera(isDad:Bool)
	{
		...
		cameraMoveTween = FlxTween.tween(camFollowPos, {x: camFollow.x, y: camFollow.y}, cameraSpeed, {/*type: FlxTween.PERSIST, */ease: FlxEase.quadInOut});
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (ratingZoomTween != null) {
			ratingZoomTween.cancel();
			sickTxt.x = 20;
			goodTxt.x = 20;
			badTxt.x = 20;
			shitTxt.x = 20;
		...
		if (note.rating == 'sick') {
			sickTxt.x = 30;
			ratingZoomTween = FlxTween.tween(sickTxt, {x: 20}, 0.4, {ease: FlxEase.quartOut});
		}

		if (note.rating == 'good') {
			goodTxt.x = 30;
			ratingZoomTween = FlxTween.tween(goodTxt, {x: 20}, 0.4, {ease: FlxEase.quartOut});
		}

		if (note.rating == 'bad') {
			badTxt.x = 30;
			ratingZoomTween = FlxTween.tween(badTxt, {x: 20}, 0.4, {ease: FlxEase.quartOut});
		}

		if (note.rating == 'shit') {
			shitTxt.x = 30;
			ratingZoomTween = FlxTween.tween(shitTxt, {x: 20}, 0.4, {ease: FlxEase.quartOut});
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		/* trace(cameraMoveTween.percent);
		if (Math.abs(camFollowPos.x - camFollow.x) < 20 && Math.abs(camFollowPos.y - camFollow.y) < 20 && !SONG.notes[curSection].mustHitSection)
			//if (cameraNoteTween != null)
			//	cameraNoteTween.pause();
			switch (note.noteData) {
				case 0:
					cameraNoteTween = FlxTween.tween(camFollowPos, {x: camFollow.x - 20, y: camFollow.y}, 0.2, {ease: FlxEase.quadOut});
				case 1:
					cameraNoteTween = FlxTween.tween(camFollowPos, {x: camFollow.x, y: camFollow.y + 20}, 0.2, {ease: FlxEase.quadOut});
				case 2:
					cameraNoteTween = FlxTween.tween(camFollowPos, {x: camFollow.x, y: camFollow.y - 20}, 0.2, {ease: FlxEase.quadOut});
				case 3:
					cameraNoteTween = FlxTween.tween(camFollowPos, {x: camFollow.x + 20, y: camFollow.y}, 0.2, {ease: FlxEase.quadOut});
